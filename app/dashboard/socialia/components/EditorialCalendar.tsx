...
const statusStyles: Record<PostStatus, string> = {
  ...
  pending_approval: "border-amber-400/30 bg-amber-500/10 text-amber-100",
  approved: "border-emerald-400/30 bg-emerald-500/10 text-emerald-100",
  scheduled: "border-violet-400/30 bg-violet-500/10 text-violet-100",
  published_simulated: "border-cyan-400/30 bg-cyan-500/10 text-cyan-100",
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        <span className="rounded-full bg-black/20 px-2 py-1 text-[10px] font-black uppercase">
          {statusLabels[post.status]}
        </span>
        {post.is_demo ? (
          <span className="rounded-full bg-amber-400/20 px-2 py-1 text-[10px] font-black uppercase text-amber-100">
            Demo
          </span>
        ) : null}
      </div>
      ...
      <form action={schedulePostFormAction} className="mt-3 grid gap-2">
        ...
        <input
          ...
          className="min-w-0 rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs text-white outline-none focus:border-violet-300"
        />
        <button className="rounded-xl bg-white/15 px-3 py-2 text-xs font-black hover:bg-white/25">
          Reprogramar
        </button>
      </form>
    </article>
    ...
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
      <div className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="flex rounded-2xl border border-white/10 bg-[#0b1024] p-1">
          ...
          <button
            ...
            className="rounded-2xl border border-white/10 px-4 py-3 text-sm font-bold hover:bg-white/10"
          >
            Anterior
          </button>
          <button
        ...
        <div className="mt-6 grid gap-2 overflow-x-auto pb-2">
          <div className="grid min-w-[920px] grid-cols-7 gap-2">
            {weekDays.map((day) => (
              <div
                key={day}
                className="rounded-2xl border border-white/10 bg-[#0b1024] px-3 py-2 text-center text-xs font-black uppercase tracking-[0.14em] text-slate-400"
              >
                {day.slice(0, 3)}
              </div>
            ))}
            ...
            {visibleDays.map((day) => {
              ...
              return (
                <div
                  key={day.toISOString()}
                  className={`min-h-[150px] rounded-2xl border border-white/10 p-3 ${
                    isCurrentMonth ? "bg-[#0b1024]" : "bg-white/[0.02] opacity-60"
                  }`}
                >
                    ...
                    {dayPosts.slice(0, 3).map((post) => (
                      ...
                      >
                        <div className="flex items-center gap-1">
                          ...
                          {post.is_demo ? (
                            <span className="rounded-full bg-amber-400/20 px-1.5 py-0.5 text-[9px] font-black text-amber-100">
                              Demo
                            </span>
                          ) : null}
                        </div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-7">
          {visibleDays.map((day, index) => {
            ...
            return (
              <div
                key={day.toISOString()}
                className="rounded-3xl border border-white/10 bg-[#0b1024] p-4"
              >
                ...
                <div className="mt-4 space-y-3">
                  {scheduledPosts.length ? (
                    ...
                    <p className="rounded-2xl border border-white/10 bg-black/20 p-3 text-xs text-slate-400">
